...
int day15star2(String input) {
  ...
  for (final pair in sbPairs) {
    ...
    final lineNwSe = Line(Point(0, sensor.y + sensor.x + dist),
        Point(size, sensor.y + sensor.x + dist - size));
    final lineNwSe2 = Line(Point(0, sensor.y + sensor.x - dist),
        Point(size, sensor.y + sensor.x - dist - size));
    linesNwSe.addAll([lineNwSe, lineNwSe2]);
    final lineSwNe = Line(Point(0, sensor.y - sensor.x + dist),
        Point(size, sensor.y - sensor.x + dist + size));
    final lineSwNe2 = Line(Point(0, sensor.y - sensor.x - dist),
        Point(size, sensor.y - sensor.x - dist + size));
